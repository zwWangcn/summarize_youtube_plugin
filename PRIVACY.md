...
- **设置**：所选 AI 服务商、模型、设置弹窗界面语言、输出语言、字幕翻译分块档位，以及用户保存的自定义 AI 提示词。
- **生成内容**：AI 返回的总结与字幕译文。

### 数据的存储位置

- API Key 保存在当前设备的 `chrome.storage.local`，不通过 `chrome.storage.sync` 同步。
- 服务商、模型、设置弹窗界面语言、输出语言、字幕翻译分块档位和自定义 AI 提示词等设置保存在 `chrome.storage.sync`；启用 Chrome 同步时，这些设置可能同步到用户登录的其他 Chrome 浏览器。
- 总结和字幕译文缓存在 `chrome.storage.local`，保留期最长为 7 天，并受数量上限控制。
- 界面偏好保存在 `chrome.storage.local`。

### 数据的传输和使用

用户配置 API Key 后，扩展会在用户主动请求总结或翻译时传输数据；若播放器双语字幕保持默认开启，也可能为当前播放位置自动预翻译字幕。扩展会把所选视频的字幕、生成指令、当前启用的自定义提示词和对应 API Key 通过 HTTPS 直接发送给用户选择的 AI 服务商。数据由该服务商依据其条款和隐私政策处理。
...
用户可以在扩展弹窗中删除当前设备保存的全部 API Key，编辑或删除自定义提示词，并可关闭默认双语字幕。卸载扩展会删除该设备上的扩展本地数据；同步设置的删除也可能受 Chrome Sync 状态影响。第三方 AI 服务商已接收的数据应按照对应服务商提供的控制方式管理或删除。
...
- **Settings**, including the selected provider, model, popup interface language, output language, caption translation chunk preset, and user-saved custom AI instructions.
...
- Provider, model, popup interface language, output language, caption translation chunk preset, and custom AI instructions are stored in `chrome.storage.sync` and may sync between Chrome browsers when Chrome Sync is enabled.
...
After the user configures an API key, data is transferred when the user requests a summary or translation. If in-player bilingual captions remain enabled by default, the extension may also pre-translate captions near the current playback position. The selected video's transcript, generation instructions, currently enabled custom instruction, and corresponding API key are sent over HTTPS directly to the selected AI provider. That provider processes the data under its own terms and privacy policy.
...
Users can delete all API keys stored on the current device, edit or delete custom instructions, and disable bilingual captions by default from the extension popup. Uninstalling the extension removes local data from that device; deletion of synced settings may also depend on Chrome Sync state. Data already received by an AI provider is subject to that provider's deletion controls.

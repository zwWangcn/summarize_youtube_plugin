...
处理时会按上下文合并自动字幕中被错误切碎的片段，并保守修正可以确定的识别错误。每个完成分段立即缓存 7 天，关闭面板或切换到总结后全文任务仍会继续；再次翻译全文时只补齐缺失部分。不会下载音频或重新校准语音时间轴，中文字幕不会调用 AI。
...
5. YouTube 外语视频可在字幕页点击 **翻译本段** 或 **翻译全文**，并用 **原文 / 译文** 切换查看

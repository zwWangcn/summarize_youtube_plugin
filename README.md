...
翻译功能内嵌于「字幕原文」。字幕按字符量自动分段，打开时定位当前播放位置，上下滚动按需加载原文；滚动不会自动调用 AI。可翻译当前分段、重新翻译单段，或点击「翻译全文」补齐尚未完成的分段，并在原文/译文之间切换。

处理时会把带时间戳的原字幕和前后文一并交给 AI，由 AI 将错误切碎的片段整理成自然句子并估算秒级开始时间，同时保守修正可以确定的识别错误。译文不要求和原始碎片一一对应。每个完成分段按目标语言独立缓存 7 天，关闭面板或切换到总结后全文任务仍会继续；再次翻译全文时只补齐缺失部分。不会下载音频或重新校准语音时间轴；源字幕已经属于目标语言时不会调用翻译 AI。
